...
- **Semantic search UI.** `RagSearchBar` (`src/components/search/rag-search-bar.tsx`) calls `rag_search.searchArticlesRAG`. Optional query history uses `localStorage` key `headsalon-search-history` (older `smart-search-history` entries are cleared on load). Parsed `word:token` filter fragments are supported; `urlSync` can wire `?q=` later.
...
- Search: client calls action `rag_search.searchArticlesRAG`; results `src/components/search/search-results.tsx`; input `src/components/search/rag-search-bar.tsx`.
