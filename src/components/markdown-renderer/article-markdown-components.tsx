...
export const articleMarkdownComponents: Components = {
  ...
  h5: ({ node, ...props }) => (
    <h5 className="text-lg font-semibold mt-5 mb-2" {...props} />
  ),
  h6: ({ node, ...props }) => (
    <h6
  ...
  ul: ({ node, ...props }) => (
    <ul className="list-disc list-outside mb-6 ml-6 space-y-2" {...props} />
  ),
  ol: ({ node, ...props }) => (
    <ol className="list-decimal list-outside mb-6 ml-6 space-y-2" {...props} />
  ),
  li: ({ node, ...props }) => (
  ...
  input: ({ node, ...props }) => {
    if (props.type === "checkbox") {
      return (
        <input className="mr-2 accent-primary" disabled {...props} />
      );
    }
    return <input {...props} />;
  },
  ...
  kbd: ({ node, ...props }) => (
    <kbd className="bg-muted border border-border rounded px-2 py-1 text-xs font-mono shadow-sm" {...props} />
  ),
  mark: ({ node, ...props }) => (
